...
from fonctions_basiques import (
    ...
    charger_zones_inondables,enrichir_donnees_inondations
)
...
def render_tab_concurrence(df_communes, df_coefficients, gdf_socio_filtre, indicateur, nom_indicateur, maille,
                           poi_selectionnes_sidebar, gdf_inondations):
...
def render_tab_implantation(gdf_socio_filtre, indicateur, nom_indicateur, maille,
    ...
    if final_lat and final_lon:
        ...
        temps_isochrones = st.slider("Temps de trajet (min) :", 2, 20, 10, 1, key="temps_implantation") if mode == 'Isochrones' else 10
        ...
        if mode == 'Isochrones':
            ...
            poi_point_gdf = gpd.GeoDataFrame(geometry=[Point(final_lon, final_lat)], crs="EPSG:4326"); poi_reproj = poi_point_gdf.to_crs("EPSG:3857")
            zone_analyse_geom = poi_reproj.buffer(radius).iloc[0]; zone_analyse_geom = gpd.GeoDataFrame(geometry=[zone_analyse_geom], crs="EPSG:3857").to_crs("EPSG:4326").geometry.iloc[0]
                ...
                if liste_gdf_poi:
                    ...
                    if liste_gdf_poi_non_vides:
                        ...
                        if not gdf_poi_brut.empty:
                            gdf_poi_trouves = gdf_poi_brut[gdf_poi_brut.within(zone_analyse_geom)]; st.info(f"{len(gdf_poi_trouves)} point(s) d'intérêt trouvé(s).")
        gdf_batiments_final = gpd.GeoDataFrame()
        if afficher_batiments and zone_analyse_geom:
            bbox_batiments = zone_analyse_geom.bounds; gdf_batiments_brut = rechercher_batiments_osm(bbox_batiments)
            if not gdf_batiments_brut.empty:
                gdf_batiments_final = gdf_batiments_brut[(gdf_batiments_brut['surface_m2'] >= surface_min) & (gdf_batiments_brut['surface_m2'] <= surface_max)]
                gdf_batiments_final = gdf_batiments_final[gdf_batiments_final.within(zone_analyse_geom)]; st.info(f"{len(gdf_batiments_final)} bâtiment(s) correspondant à vos critères.")
        st.markdown("---"); st.subheader("Carte Interactive de la Zone")
        map_object, legend_socio_color, legend_socio_single = creer_carte_implantation(
            ...
            gdf_batiments=gdf_batiments_final, gdf_inondations=gdf_inondations
        )
        col_carte, col_legende = st.columns([3, 1])
        with col_carte:
            st_folium(map_object, width=800, height=600, returned_objects=[])
        with col_legende:
            ...
            if not gdf_inondations.empty:
                st.write("**Risque Inondation**")
                st.markdown(f'<span style="color:#fdbb84; font-size:22px;">●</span> Aléa Faible', unsafe_allow_html=True)
                st.markdown(f'<span style="color:#e34a33; font-size:22px;">●</span> Aléa Moyen', unsafe_allow_html=True)
                st.markdown(f'<span style="color:#b30000; font-size:22px;">●</span> Aléa Fort', unsafe_allow_html=True)
            if legend_socio_color or legend_socio_single:
                st.markdown("<hr style='margin:0.5em 0;'>", unsafe_allow_html=True)
            if legend_socio_color:
                ...
                c1, c2 = st.columns(2); c1.markdown(f"<small>{legend_socio_color.vmin:,.0f}".replace(",", " ") + "</small>", unsafe_allow_html=True); c2.markdown(f'<div style="text-align: right;"><small>{"{:,}".format(round(legend_socio_color.vmax)).replace(",", " ")}</small></div>', unsafe_allow_html=True)
            elif legend_socio_single:
                st.write(f"**{legend_socio_single['label']}**"); st.markdown(f"Valeur unique : **{'{:,.0f}'.format(legend_socio_single['value']).replace(',', ' ')}**")
# =============================================================================
# FONCTION PRINCIPALE DE LA PAGE
# =============================================================================
def page_osm(path_communes, path_iris_socio, path_coeff_trafic, path_zones_inondables):
    ...
    with st.spinner("Chargement des données initiales..."):
        ...
        gdf_inondations_base = charger_zones_inondables(path_zones_inondables)

    gdf_inondations = enrichir_donnees_inondations(gdf_inondations_base, df_communes)
    ...
    if risque_selectionne == "Inondations" and not gdf_inondations.empty:
        if regions_filtrees and 'NOM_REG' in gdf_inondations.columns:
            gdf_inondations['NOM_REG_Upper'] = gdf_inondations['NOM_REG'].str.upper().str.replace('-', ' ')
            regions_filtrees_upper = [r.upper().replace('-', ' ') for r in regions_filtrees]
            gdf_inondations_a_afficher = gdf_inondations[gdf_inondations['NOM_REG_Upper'].isin(regions_filtrees_upper)]

        elif departements_filtres and 'Num_Dep' in gdf_inondations.columns and 'NOM_DEP' in gdf_inondations.columns:
            # RENOMMÉ : On crée et filtre sur la colonne 'affichage_dep'
            gdf_inondations['affichage_dep'] = gdf_inondations['Num_Dep'] + " - " + gdf_inondations['NOM_DEP']
    ...
    with tab_concurrence:
        render_tab_concurrence(df_communes, df_coefficients, gdf_socio_filtre, indicateur, nom_indicateur, maille,
                               poi_selectionnes_sidebar, gdf_inondations_a_afficher)
